...
- **BadgerDB** — встраиваемая NoSQL БД для хранения блокчейна
...
Для обеспечения безопасного соединения с использованием TLS необходимо подготовить сертификаты в директории:
```
/blockchain/certs
```
...
Если вы используете клиент, который обращается к серверу по HTTPS (например, `curl`, Postman или отдельный клиентский сервис), **уберите** следующую строку из клиентской части (или из кода, где инициализируется HTTP-клиент). Эта опция отключает проверку сертификата сервера и **должна использоваться только в тестовой среде**. Использование этого флага в продакшене **сильно снижает безопасность** и делает систему уязвимой к MITM-атакам.
